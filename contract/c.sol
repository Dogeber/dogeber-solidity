...
Telegram: https://t.me/MetaMorse
Website:  https://metamorse.site/
*/
...
contract Meta_Morse is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "Meta Morse";
    string private _symbol = "MEMO";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0x3E0ABcF47E50519A75F535b947CD737f78463cCE);
    address payable public DevelopmentWalletAddress = payable(0x0A96942d172b3087bfE388852E415C0512750920);
    address payable public LiquidityWalletAddress = payable(0x3E0ABcF47E50519A75F535b947CD737f78463cCE);
    address payable public AdminWalletAddress = payable(0x28b2e2D33a767828a279182886c1dd3598CC0687);
    ...
    uint MAX_TAX_LIMIT = 25;
    uint MIN_TAX_LIMIT = 10;
    ...
    uint256 public _marketingShare = 14;
    uint256 public _DevShare = 4;
    ...
    //100,000,000,000
    uint256 private _totalSupply = 100000000000 * 10**_decimals;   //100 Billion
    //1,000,000,000
    uint256 public _maxBuyAmount = 1000000000 * 10**_decimals;     // 1 Billion
    //500,000,000  (Anti-Whale)
    uint256 public _maxSellAmount = 500000000 * 10**_decimals;   //500 Million
    // 3%
    ...
    constructor () {
        ...
        isExcludedFromFee[AdminWalletAddress] = true;
        ...
        isWalletLimitExempt[AdminWalletAddress] = true;
        isWalletLimitExempt[address(uniswapPair)] = true;
        isWalletLimitExempt[address(this)] = true;

        isBuyLimitExempt[AdminWalletAddress] = true;
        isBuyLimitExempt[address(this)] = true;

        isSellLimitExempt[AdminWalletAddress] = true;
        isSellLimitExempt[address(this)] = true;

        isMarketPair[address(uniswapPair)] = true;

        transferOwnership(AdminWalletAddress);
        _balances[AdminWalletAddress] = _totalSupply;
        emit Transfer(address(0), AdminWalletAddress, _totalSupply);
    }
    ...
    function setSellLimit(uint256 _value) external onlyOwner{
        uint256 restrictedLimit = 500000000 * 10**_decimals;  //500 Million
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBTeam = amountReceived.mul(_DevShare).div(totalBNBFee);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            if(!isBuyLimitExempt[recipient]){
                require(amount <= _maxBuyAmount,"Amount of Buy Exceeded from Max Limit");
            }
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(isMarketPair[recipient]) {
            if(!isSellLimitExempt[sender]){
                require(amount <= _maxSellAmount,"Amount of Sell Exceeded from Max Limit");
            }
