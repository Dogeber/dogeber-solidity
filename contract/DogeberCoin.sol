...
/**
* Main Website:  http://www.dogeber.org
* Web3 Social Network System:  http://www.dogeber.com
...
contract DogeBer is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public marketingAddress = payable(0x1F0f7A5DC9680c71fF6034E9Ec661Bea59294D98);
    address payable public reserveWallet = payable(0xd37478bbfbC0c921Eb26Af99e3ae4acc0901b481);
    ...
    uint256 public totalBuyFees = marketingBuyFees.add(reserveBuyFee).add(liquidityBuyFee).add(RewardsBuyFee);
    uint256 public totalSellFees = marketingSellFees.add(reserveSellFee).add(liquiditySellFee).add(RewardsSellFee);
    uint256 public totalTransferFees = marketingTransferFees.add(reserveTransferFee).add(liquidityTransferFee).add(RewardsTransferFee);
    ...
    constructor() ERC20("Dogeber Coin", "DOGEBER") {
    ...
    function updateBuyFees(
        ...
        uint256 _totalBuyFees = newMarketingFee.add(newReserveFee).add(newLiquidityFee).add(newRewardsFee);
        ...
        totalBuyFees = marketingBuyFees.add(reserveBuyFee).add(liquidityBuyFee).add(RewardsBuyFee);
        emit UpdateBuyFees(newMarketingFee, newReserveFee, newLiquidityFee, newRewardsFee);
    ...
    function updateSellFees(
        ...
        uint256 _totalSellFees = newMarketingFee.add(newReserveFee).add(newLiquidityFee).add(newRewardsFee);
        ...
        totalSellFees = marketingSellFees.add(reserveSellFee).add(liquiditySellFee).add(RewardsSellFee);
        emit UpdateSellFees(newMarketingFee, newReserveFee ,newLiquidityFee, newRewardsFee);
    ...
    function updateTransferFees(
        ...
        uint256 _totalTransferFees = newMarketingFee.add(newReserveFee).add(newLiquidityFee).add(newRewardsFee);
        ...
        totalTransferFees = marketingTransferFees.add(reserveTransferFee).add(liquidityTransferFee).add(RewardsTransferFee);
        emit UpdateTransferFees(newMarketingFee, newReserveFee, newLiquidityFee, newRewardsFee);
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 fees = marketingFeeAmount + reserveFeeAmount + liquidityFeeAmount + RewardsFeeAmount;
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public pure override {
        require(false, "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DogeBer contract.");
    }
